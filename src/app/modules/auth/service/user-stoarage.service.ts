...
static getUser(): any {
  ...
  static grtUserId(): string {                      // (There's a typo, should be "getUserId")
    ...
    if (user == null) { return ''; }                // If no user is found, return an empty string
    return user.id;                                 // Otherwise, return the user ID
  }
  ...
  static grtUserRole(): string {               // (There's a typo, should be "getUserRole")
    ...
    const user = this.getUser();              // Get the user object
    if (user == null) { return ''; }        // If no user is found, return an empty string
    return user.role;                       // Otherwise, return the user role
  }
